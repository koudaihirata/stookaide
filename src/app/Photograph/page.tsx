...
export default function Photograph() {
    ...
    return (
        <main>
            ...
            <img id="videoFeed" src="http://localhost:5001/detect" alt="ビデオフィード" className={css({width:'90%',height:'300px',rounded:'15px',margin:'0 auto',pt:'5px'})} />
            <h3 className={css({fontSize:'18px',fontWeight:'bold',textAlign:'center',pt:'5px'})}>余っている食材達</h3>
            <ul className={css({width:'80%',height:'100px',margin:'0 auto',rounded:'8px',border:'3px solid #FFCE7B',p:'5px',display:'flex',flexWrap:'wrap',gap:'8px'})}>
                {Array.from(detectedObjects).map((obj, index) => 
                    <li key={index} className={css({border:'1px solid #FFCE7B',display:'inline-block',rounded:'12px',height:'24px'})}>
                        <div className={css({display:'flex',alignItems:'center',gap:'3px',flexWrap:'wrap'})}>
